...
def separate_vocals_and_music(audio_file):
    output_dir = 'output'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    subprocess.run(['spleeter', 'separate', '-i', audio_file, '-p', 'spleeter:2stems', '-o', output_dir])
    background_music_file = os.path.join(output_dir, 'audio', 'accompaniment.wav')
    if not os.path.exists(background_music_file):
        raise FileNotFoundError(f"{background_music_file} not found.")
...
def combine_audio_with_video(video_file, background_music_file, combined_audio):
    try:
        video_clip = mp.VideoFileClip(video_file)
        background_music_clip = mp.AudioFileClip(background_music_file)
        translated_audio_clip = mp.AudioFileClip(combined_audio)
        
        final_audio = mp.CompositeAudioClip([background_music_clip, translated_audio_clip])
...
def main():
    ...
    background_music_file = separate_vocals_and_music(mono_audio_file)
    ...
    output_video = combine_audio_with_video(video_with_captions, background_music_file, combined_audio)
    ...
    try:
        ...
        os.remove(background_music_file)
